      ...
      >
        {text.split("").map((letter, index) => (
          <motion.span
            ...
            transition={{
              duration: 0.2,
              delay: lineIndex * 0.5 + index * 0.05,
            }} // 每个字母延迟 0.05 秒
  ...
  return (
    <div className="bg-[#F7E4DF] w-full h-[630px] flex flex-col justify-between relative">
      ...
      <div className=" w-full h-full pt-12 flex flex-col items-center">
        ...
        <p className="text-[#103362] font-enFont mt-10">
        ...
        <p className=" font-jpFont text-4xl mt-4">
          <LetterWithAnimate />
        </p>
        <motion.h2
      ...
      <motion.div
        initial={{ scale: 2, opacity: 0 }} // 初始状态
        whileInView={{ scale: 1, opacity: 1 }} // 在视口内时的状态
        viewport={{ once: true }} // 只在第一次进入视口时触发
        transition={{
      ...
      <motion.div
        ...
        whileInView={{ opacity: 1 }} // 在视口内时的状态
        viewport={{ once: true }} // 只在第一次进入视口时触发
        transition={{ duration: 2 }}
      >
        <motion.div
          initial={{ scale: 0.01, opacity: 0 }} // 初始状态
          whileInView={{ scale: 1, opacity: 1 }} // 在视口内时的状态
          viewport={{ once: true }} // 只在第一次进入视口时触发
          transition={{
            type: spring,
